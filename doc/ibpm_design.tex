...
x = omega(ind);
	\end{lstlisting}
	\item \code|void curl(const Flux& q)|\\
	Set \code|*this| (the current Scalar) to the curl of q.
	\item \code|void sinTransform(const Scalar& gamma)|\\
	Set \code|*this| to the discrete sin transform of the argument.  (Assumes different chunks of memory.)
	\item \code|void sinTransformInv(const Scalar& gamma)|\\
	Set \code|*this| to the inverse discrete sin transform of the argument.
	\item \code|void laplacian(const Scalar& gamma)|\\
	Set \code|*this| to the Laplacian of the argument.
	\item \code|void laplacianInverse(const Scalar& gamma)|\\
	Assume zero boundary conditions for now (not for multiple grids).
	\item \code|void divergence(const Flux& q)|\\
	Set \code|*this| to the divergence of the argument.
...
q.y(i,j);   // i in [0,m-1]
		    // j in [0,n]
\end{lstlisting}
	\item \code|void curl(const Scalar& f)|\\
	Set \code|*this| to the curl of the argument.
	\item \code|void gradient(const Scalar& f)|\\
	Set \code|*this| to the gradient of the argument.
