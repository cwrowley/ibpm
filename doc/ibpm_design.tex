...
\begin{description}
	\item \fn{NavierStokesModel}(Grid grid, Geometry geom)\\
		Constructor.  Set up the appropriate data needed to evaluate $\Lambda$, and the discrete sin transform $S$, $S^{-1}$.
...
\subsection{State}
Include time, scalar, flux, and force.
...
\subsection{Probe}
\subsection{Geometry}
Include the name of the geometry, to be saved by the \class{CholeskySolver} when a Cholesky factorization is needed.
